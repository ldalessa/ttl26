...
namespace ttl
{
    ...
    struct tspan : public std::mdspan<T, Extents, LayoutPolicy, AccessorPolicy>
    {
        ...
        /// themselves bound. This allows us to write custom evaluation
        /// code for mdspan and have it work properly for things bound
        /// via the tspan.
        ///
        /// Copying the mdspan is going to be cheap since it's non-
        /// owning.
    ...
    /// Used to expand type packs to values.
    template <class T, auto V>
    inline constexpr auto _t2v = V;
    ...
    tspan(R&&, Extents const&) -> tspan<
        std::remove_reference_t<std::ranges::range_reference_t<R>>,
        Extents>;

    /// Infer T as the range value type, and the dynamic extents.
    template <std::ranges::contiguous_range R, std::integral I, std::integral... J>
    tspan(R&&, I, J...) -> tspan<
        std::remove_reference_t<std::ranges::range_reference_t<R>>,
        std::extents<std::size_t, std::dynamic_extent, _t2v<J, std::dynamic_extent>...>
    >;

    /// Infer T as the iterator value type.
    template <std::contiguous_iterator It, std_extents Extents>
    tspan(It const&, Extents const&) -> tspan<
    ...
    tspan(It const&, I...) -> tspan<
        std::remove_reference_t<std::iter_reference_t<It>>,
        std::extents<std::size_t, _t2v<I, std::dynamic_extent>...>
    >;
}
