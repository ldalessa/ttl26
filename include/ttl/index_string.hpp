...
namespace ttl
{
    ...
    /// index_string. A projected index is one where the user has
    /// specified an integer. For example A(i,2) is projecting the
    /// 3rd column of A, and the corresponding index_string will be
    /// "i*".
    ...
    struct index_string
    {
        ...
        {
            // std::strcpy is not constexpr, so we use our cstring
            // sentinel and use copy
        ...
        /// @note(performance)
        ///
        /// I have chosen to store the string as a null-terminated
        /// string because then compiler and debugger output prints the
        /// string rather than an array of characters. I have chsen not
        /// to store the size because it is redundant and it needs to be
        /// kept consistent and makes various operations more complicated.
        ///
        /// Strings are short and this is all done in constexpr, so I'm
        /// relatively happy with this decision. It's something that could
        /// be re-evaluated if compilation times become extreme.
        ...
        /// projected. Contracted indices are those that are not projected
        /// and appear twice in _data. Projected indices are those that
        /// are the projected_index (currently `'*'`).
        ...
        ///
        /// The resulting string is N + M - 1 because we need at most 1
        /// '\0' and N + M has two.
        template <std::size_t M>
        ...
        /// This set of query functions creates new index_string<N>
        /// that contain the requested subset of indices. We do these
        /// explicitly because we know they can never get any larger
        /// than N, so this prevents N explosion during concatenation.
        /// @{
        ...
        /// Find the index of `c`.
        ///
        /// This returns the index of the first instance of `c`. If
        /// `c` is contracted or projected this may not be what
        /// the caller wants.
        ...
        ///
        /// If `c` is not the projected index '*' then this is the same
        /// as index_of(c). If `c` is the projected index then this finds
        /// the pth indstance of it and returns that index, and updates
        /// `p` by one.
        ///
        /// The point is that this can be used transparently in a loop that
        /// is tracking projected offsets.
        ///
        ...
        ///
        /// @returns The found index, and updates `p` if `c` was the proejcted
        /// index.
    ...
    private:
        ...
        ///
        /// Outer indices are those that are not '*' and only appear
        /// once. These are indices that are neither projected nor
        /// contracted.
        ///
        ...
        ///
        /// Contracted indices are those that are not '*' and appear
        /// twice. Each contracted index must be added only once to the
        /// output iterator. In order to check this, the output
        /// `index_string` corresponding to the `out` iterator is also
        /// provided.
        ///
        ...
        ///
        /// Projected indices are those corresponding to '*'. Every
        /// instance of '*' is coppied to the output.
        ///
        /// @param out The output iterator.
        ///
        /// @returns The updated `out` iterator.
